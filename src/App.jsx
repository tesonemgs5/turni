...
function loadStore(){
  try{ const r=localStorage.getItem(SK); return r?JSON.parse(r):INIT; }
  catch{ return INIT; }
}
function saveStore(s){
  try{ localStorage.setItem(SK,JSON.stringify(s)); }catch(e){ console.log(e); }
}
...
export default function App(){
  ...
  const [calId, setCalId] = useState(()=>loadStore().calendars[0]?.id||null);
  ...
  function save(ns){ setStore(ns); saveStore(ns); }
  ...
  function saveEvt(){
    ...
    const evt={id:uid(),color,label,note:form.note||"",
      allDay:form.dur==="allday",tIn:form.dur==="allday"?"":form.tIn||"",
      tOut:form.dur==="allday"?"":tOut,place:form.place||"",map:form.map||""};
    ...
    save(ns); setForm(null); setDayKey(null);
  }
                  ...
                  <div style={{flex:1,overflow:"hidden",display:"flex",flexDirection:"column",gap:"1px",padding:"0 1px 1px"}}>
                    {evts.slice(0,4).map((e,ei)=>(
                      <div key={e.id+ei} style={{background:e.color,borderRadius:3,padding:"2px 4px",
                        ...
                        whiteSpace:"nowrap",lineHeight:"13px",flexShrink:0,textShadow:"0 1px 2px rgba(0,0,0,0.35)"}}>
                        {e.label}
                      </div>
                    ))}
                    {evts.length>4&&<div style={{fontSize:8,color:T.sub,padding:"0 2px"}}>+{evts.length-4}</div>}
                  </div>
                </div>
              );
            })}
      ...
      {screen==="settings"&&(
        <div style={{flex:1,overflowY:"auto",padding:"12px 12px 80px",color:T.text}}>
          ...
          <Sec label="TEMA" T={T}>
            <div style={{display:"flex",gap:6}}>
              {[["auto","Auto"],["light","Chiaro"],["dark","Scuro"]].map(([v,l])=>(
                <button key={v} onClick={()=>save({...store,theme:v})}
          ...
          <Sec label="CALENDARI" T={T}>
                ...
                <div style={{display:"flex",alignItems:"center",gap:8,background:T.s2,borderRadius:10,padding:"8px 10px"}}>
                  <div style={{position:"relative",flexShrink:0}}>
                    ...
                    {pal===c.id&&<Pal T={T} cur={c.color} onPick={p=>{
                      const ns=JSON.parse(JSON.stringify(store));ns.calendars[ci].color=p;save(ns);setPal(null);
                    }}/>}
                  </div>
                  <input value={c.name}
                    onChange={e=>{const ns=JSON.parse(JSON.stringify(store));ns.calendars[ci].name=e.target.value;save(ns);}}
                    style={{flex:1,background:"transparent",border:"none",outline:"none",color:T.text,fontSize:13,fontWeight:700}}/>
                  <button onClick={()=>{const ns=JSON.parse(JSON.stringify(store));
                    ns.calendars=ns.calendars.map((x,j)=>({...x,isMain:j===ci}));save(ns);}}
                  ...
                  <button onClick={()=>setExCal(exCal===c.id?null:c.id)}
                    ...
                    {exCal===c.id?"▲":"▼"}</button>
                  <button onClick={()=>{const ns=JSON.parse(JSON.stringify(store));
                    ns.calendars=ns.calendars.filter(x=>x.id!==c.id);save(ns);}}
                  ...
                  <div style={{background:T.s2,borderRadius:"0 0 10px 10px",padding:"10px",borderTop:`1px solid ${T.border}`}}>
                      ...
                      <div key={sh.id} style={{display:"flex",alignItems:"center",gap:6,marginBottom:6}}>
                        <div style={{position:"relative",flexShrink:0}}>
                          ...
                          {pal===sh.id&&<Pal T={T} cur={sh.color} onPick={p=>{
                            const ns=JSON.parse(JSON.stringify(store));ns.calendars[ci].shifts[si].color=p;save(ns);setPal(null);
                          }}/>}
                        </div>
                        <input value={sh.label}
                          onChange={e=>{const ns=JSON.parse(JSON.stringify(store));
                            ns.calendars[ci].shifts[si].label=e.target.value;save(ns);}}
                          style={{flex:1,background:"transparent",border:"none",outline:"none",color:T.text,fontSize:12}}/>
                        <button onClick={()=>{const ns=JSON.parse(JSON.stringify(store));
                          ns.calendars[ci].shifts=ns.calendars[ci].shifts.filter((_,k)=>k!==si);save(ns);}}
                    ...
                    <div style={{display:"flex",gap:6,alignItems:"center",marginTop:6}}>
                      ...
                      <button onClick={()=>{
                        ...
                        const ns=JSON.parse(JSON.stringify(store));
                        if(!ns.calendars[ci].shifts) ns.calendars[ci].shifts=[];
                        ns.calendars[ci].shifts.push({id:uid(),label:nsName.trim(),color:nsColor});
                        save(ns); setNsName("");
            ...
            <div style={{display:"flex",gap:8,alignItems:"center",marginTop:4}}>
              ...
              <button onClick={()=>{
                ...
                save({...store,calendars:[...store.calendars,
          ...
          <Sec label="FESTIVI LOCALI" T={T}>
            <div style={{fontSize:11,color:T.sub,marginBottom:8}}>Domeniche e festivi italiani già in rosso automaticamente.</div>
            {(store.extraHols||[]).map((h,i)=>(
              <div key={i} style={{display:"flex",alignItems:"center",gap:8,
                ...
                <button onClick={()=>save({...store,extraHols:(store.extraHols||[]).filter((_,j)=>j!==i)})}
                  style={{background:"none",border:"none",color:"#ef4444",cursor:"pointer",fontSize:16}}>×</button>
              </div>
            ))}
            <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
              ...
              <button onClick={()=>{
                ...
                save({...store,extraHols:[...(store.extraHols||[]),{name:nhName.trim(),d:nhD,m:nhM}]});
          ...
          <div style={{background:T.surface,borderRadius:"18px 18px 0 0",width:"100%",
            ...
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
              ...
              <div style={{display:"flex",gap:8}}>
                {!form&&activeCal&&(
                  <button onClick={()=>setForm({shiftId:null,label:"",note:"",dur:"allday",tIn:"",tOut:"",place:"",map:"",colorOvr:null})}
                    style={{background:accent,border:"none",borderRadius:8,color:"#fff",
                      fontSize:13,fontWeight:800,padding:"7px 14px",cursor:"pointer"}}>+ Aggiungi</button>
                )}
                <button onClick={()=>{setDayKey(null);setForm(null);}}
                  style={{background:T.s2,border:"none",borderRadius:8,color:T.sub,width:32,height:32,cursor:"pointer",fontSize:18}}>×</button>
              </div>
            </div>
            {/* Cal switcher */}
                ...
                return (
                  <button key={c.id} onClick={()=>setCalId(c.id)}
                    style={{display:"flex",alignItems:"center",gap:4,cursor:"pointer",
                      ...
                      border:`1.5px solid ${calId===c.id?c.color:T.border}`,borderRadius:8,padding:"3px 10px"}}>
                    <div style={{width:8,height:8,borderRadius:"50%",background:c.color}}/>
                    <span style={{fontSize:11,color:T.text,fontWeight:600}}>{c.name}{n>0?` (${n})`:""}</span>
                  </button>
                );
              })}
            </div>
            {/* Events */}
            {getEvts(dayKey,calId).length===0&&!form&&(
              <div style={{textAlign:"center",color:T.sub,padding:"24px 0",fontSize:13}}>
                Nessun evento — premi + Aggiungi
              </div>
            )}
            {getEvts(dayKey,calId).map(e=>(
              <div key={e.id} style={{background:e.color,borderRadius:10,padding:"10px 12px",marginBottom:8,
                display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
                <div style={{flex:1}}>
                  <div style={{color:"#fff",fontSize:14,fontWeight:800,textShadow:"0 1px 3px rgba(0,0,0,0.3)"}}>{e.label}</div>
                  {!e.allDay&&e.tIn&&<div style={{color:"rgba(255,255,255,0.85)",fontSize:11,marginTop:2}}>🕐 {e.tIn}{e.tOut?` → ${e.tOut}`:""}</div>}
                  {e.note&&<div style={{color:"rgba(255,255,255,0.8)",fontSize:11,marginTop:3}}>{e.note}</div>}
                  {e.place&&(e.map
                    ?<a href={e.map} target="_blank" rel="noreferrer" style={{color:"#fff",fontSize:11,marginTop:3,display:"block"}}>📍 {e.place}</a>
              ...
              <div style={{background:T.s2,borderRadius:12,padding:14,marginTop:8}}>
                  ...
                  <>
                    ...
                    <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:6,marginBottom:12}}>
                      {activeCal.shifts.map(s=>(
                        <button key={s.id} onClick={()=>setForm(f=>({...f,shiftId:s.id,label:s.label,colorOvr:null}))}
                ...
                <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:10}}>
                  ...
                  <div style={{position:"relative"}}>
                    ...
                    {pal==="ec"&&<Pal T={T} up onPick={p=>{setForm(f=>({...f,colorOvr:p}));setPal(null);}}/>}
                  </div>
                  {form.colorOvr&&<button onClick={()=>setForm(f=>({...f,colorOvr:null}))}
                  ...
                  {[["allday","Tutto il giorno"],["fixed","6h 15m"],["custom","Orario libero"]].map(([v,l])=>(
                    <button key={v} onClick={()=>setForm(f=>({...f,dur:v}))}
                      style={{flex:1,padding:"7px 2px",borderRadius:8,cursor:"pointer",fontSize:10,fontWeight:700,
                        background:form.dur===v?accent:T.surface,color:form.dur===v?"#fff":T.sub,
                ...
                {form.dur!=="allday"&&(
                  <div style={{display:"flex",gap:8,marginBottom:10}}>
                    <div style={{flex:1}}>
                      ...
                      <input type="time" value={form.tIn||""} onChange={e=>setForm(f=>({...f,tIn:e.target.value}))}
                        style={{width:"100%",background:T.surface,border:`1px solid ${T.border}`,
                    ...
                    {form.dur==="custom"&&(
                      <div style={{flex:1}}>
                        ...
                        <input type="time" value={form.tOut||""} onChange={e=>setForm(f=>({...f,tOut:e.target.value}))}
                          style={{width:"100%",background:T.surface,border:`1px solid ${T.border}`,
                            borderRadius:8,padding:"7px 8px",color:T.text,fontSize:13,outline:"none"}}/>
                      </div>
                    )}
                    {form.dur==="fixed"&&form.tIn&&(
                      <div style={{flex:1}}>
                        ...
                        <div style={{background:T.s2,border:`1px solid ${T.border}`,borderRadius:8,padding:"7px 8px",fontSize:13,color:T.sub}}>
                          {(()=>{const[h,m]=form.tIn.split(":").map(Number);const t=h*60+m+375;
                ...
                <input value={form.place||""} onChange={e=>setForm(f=>({...f,place:e.target.value}))}
                  ...
                  style={{width:"100%",background:T.surface,border:`1px solid ${T.border}`,
                    borderRadius:8,padding:"8px 10px",color:T.text,fontSize:12,marginBottom:6,boxSizing:"border-box",outline:"none"}}/>
                {form.place&&<input value={form.map||""} onChange={e=>setForm(f=>({...f,map:e.target.value}))}
                ...
                <input value={form.note||""} onChange={e=>setForm(f=>({...f,note:e.target.value}))}
                  placeholder="Note..."
                  style={{width:"100%",background:T.surface,border:`1px solid ${T.border}`,
                    borderRadius:8,padding:"8px 10px",color:T.text,fontSize:12,marginBottom:12,boxSizing:"border-box",outline:"none"}}/>
                <div style={{display:"flex",gap:8}}>
                  <button onClick={()=>{setForm(null);setPal(null);}}
                    style={{flex:1,background:T.surface,border:`1px solid ${T.border}`,
                      borderRadius:10,color:T.sub,padding:"11px 0",cursor:"pointer",fontSize:13,fontWeight:700}}>Annulla</button>
                  <button onClick={saveEvt}
                    style={{flex:2,background:accent,border:"none",borderRadius:10,
                      color:"#fff",padding:"11px 0",cursor:"pointer",fontSize:13,fontWeight:800}}>💾 Salva</button>
                </div>
              </div>
            )}
          </div>
...
function Pal({T,cur,onPick,up=false}){
  return(
    <div style={{position:"absolute",zIndex:500,...(up?{bottom:30,left:0}:{top:30,left:0}),
      background:T.surface,border:`1px solid ${T.border}`,borderRadius:12,padding:10,
      boxShadow:"0 8px 32px rgba(0,0,0,0.25)"}} onClick={e=>e.stopPropagation()}>
      <div style={{display:"grid",gridTemplateColumns:"repeat(6,1fr)",gap:6}}>
        {PALETTE.map(p=>(
          <div key={p} onClick={()=>onPick(p)}
            style={{width:24,height:24,borderRadius:"50%",background:p,cursor:"pointer",
              outline:cur===p?`3px solid ${p==="#ffffff"?"#000":"#fff"}`:"none",outlineOffset:2}}/>
        ))}
      </div>
    </div>
  );
...
const NB={background:"none",border:"none",fontSize:22,cursor:"pointer",padding:"0 4px",lineHeight:1,flexShrink:0,color:"rgba(255,255,255,0.8)"};
